...
- **`stream_service.dart`**: Central ChangeNotifier orchestrating background service lifecycle (`startBroadcast`, `stopBroadcast`), managing streaming state transitions, telemetry, remote hardware state (flash, quality, target FPS, audio status) synchronized with the server, as well as 24/7 persistence controls (`isIgnoringBatteryOptimizations`, `requestIgnoreBatteryOptimizations`, `isAutoStartEnabled`, `setAutoStartEnabled`).
